...
class FallbackConversationAgent(conversation.ConversationEntity, conversation.AbstractConversationAgent):
    ...
    async def _process_with_summary(self, user_input, agent_manager, agents, agent_names, default_agent, conversation_mode):
        ...
        summary_agent = agents[-1] if len(agents) > 1 else None
...
<SUMMARY_SECTION>
在这里直接提供你的最终答案，不要使用模板文字，而是用你自己的语言回答用户的问题。
</SUMMARY_SECTION>

严格注意：
- 你的回复必须包含且仅包含上述两个部分，并使用指定的标记，必须中文！禁止`json`格式！
- 不要在回复中添加任何其他前言、说明或额外内容
- 不要使用"这里是我的分析"等引导语
- 分析部分必须在代码块中，总结部分必须在标记内
- 总结应该综合各个AI的观点，并添加你自己的见解
- 最后是你的总结答案，最终更新所有AI的答案生成根据用户提问和结合双方你的自己的回复答案
...
"""
        ...
        try:
            ...
            if conversation_mode == CONVERSATION_MODE_NO_NAME:
                ...
                result.response.speech['plain']['speech'] = f"({summary_agent_name}) 回复: {summary_part if summary_part else response_text}"
            elif conversation_mode == CONVERSATION_MODE_DETAILED:
                detailed_response = ""
                ...
                if summary_part:
                    detailed_response += f"({summary_agent_name}) 回复: {summary_part}"
                else:
                    detailed_response += f"({summary_agent_name}) 回复: {response_text}"
    ...
    async def _process_with_fallback(self, user_input, agent_manager, agents, agent_names, default_agent, conversation_mode):
        ...
        if agents:
            first_agent_id = agents[0]
            if not isinstance(first_agent_id, str):
                if hasattr(first_agent_id, "__class__") and first_agent_id.__class__.__name__ == "DefaultAgent":
                    first_agent_id = conversation.const.HOME_ASSISTANT_AGENT
                else:
                    try:
                        first_agent_id = str(first_agent_id)
                    except:
                        first_agent_id = conversation.const.HOME_ASSISTANT_AGENT
            
            try:
                first_agent = conversation.agent_manager.async_get_agent(self.hass, first_agent_id)
                first_result = await first_agent.async_process(user_input)
                
                if (first_result and 
                    first_result.response and 
                    first_result.response.speech and 
                    'plain' in first_result.response.speech and
                    first_result.response.speech['plain'].get('speech') and
                    first_result.response.response_type == intent.IntentResponseType.ACTION_DONE):
                    
                    response_text = first_result.response.speech['plain']['speech']
                    agent_name = agent_names.get(first_agent_id, "UNKNOWN")
                    if first_agent_id == conversation.const.HOME_ASSISTANT_AGENT:
                        agent_name = default_agent.name
                    
                    first_result.response.speech['plain']['original_speech'] = response_text
                    first_result.response.speech['plain']['agent_name'] = agent_name
                    first_result.response.speech['plain']['agent_id'] = first_agent_id
                    
                    if conversation_mode == CONVERSATION_MODE_NO_NAME:
                        first_result.response.speech['plain']['speech'] = response_text
                    elif conversation_mode == CONVERSATION_MODE_ADD_NAME:
                        first_result.response.speech['plain']['speech'] = f"({agent_name}) 回复: {response_text}"
                    elif conversation_mode == CONVERSATION_MODE_DETAILED:
                        first_result.response.speech['plain']['speech'] = f"({agent_name}) 回复: {response_text}"
                    
                    self._attr_chat_response = first_result.response.speech['plain']['speech']
                    self.async_write_ha_state()
                    asyncio.create_task(self._call_speaker_service(first_result.response.speech['plain']['speech']))
                    return first_result
            except Exception as e:
                pass
        ...
        for agent_id in agents:
            ...
            result = await self._async_process_agent(
                ...
                agent_name,
                user_input,
                CONVERSATION_MODE_NO_NAME if conversation_mode == CONVERSATION_MODE_DETAILED else conversation_mode,
                result if conversation_mode != CONVERSATION_MODE_DETAILED else None,
            )
            ...
            if conversation_mode == CONVERSATION_MODE_DETAILED:
                detailed_responses.append({
                    ...
                    "is_error": result.response.response_type == intent.IntentResponseType.ERROR or
                              response_text in DEFAULT_ERROR_RESPONSES or
                              not response_text or
                              len(response_text) <= 10 or
                              response_text.lower().startswith('python')
                })
                if (response_text and 
                    not response_text.lower().startswith('python') and
        ...
        if conversation_mode == CONVERSATION_MODE_DETAILED and detailed_responses:
            detailed_output = ""
            for resp in detailed_responses:
                detailed_output += f"({resp['agent_name']}) 回复: {resp['response']}\n\n"
            
            if final_result:
                final_result.response.speech['plain']['speech'] = detailed_output
                self._attr_chat_response = detailed_output
                self.async_write_ha_state()
                asyncio.create_task(self._call_speaker_service(final_result.response.speech['plain']['speech']))
                return final_result
            else:
                intent_response = intent.IntentResponse(language=user_input.language)
                intent_response.async_set_speech(detailed_output)
                ...
                self._attr_chat_response = detailed_output
                self.async_write_ha_state()
                asyncio.create_task(self._call_speaker_service(detailed_output))
                return result
    ...
    def _create_error_response(self, user_input, all_results, conversation_mode):
        ...
        if conversation_mode == CONVERSATION_MODE_DETAILED and all_results:
            for res in all_results:
                if res and hasattr(res, 'response') and res.response:
                    if hasattr(res.response, 'speech') and 'plain' in res.response.speech:
                        r = res.response.speech['plain']
                        response_text += f"\n({r.get('agent_name', 'UNKNOWN')}) 回复: {r.get('original_speech', r['speech'])}"
        
        if not response_text:
            response_text = "抱歉，我无法理解您的请求。请尝试重新表述或使用其他命令。"
