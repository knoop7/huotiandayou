...
async def async_migrate_entry(hass, config_entry: ConfigEntry):
    ...
    LOGGER.debug("迁移自版本 %s", config_entry.version)
    if config_entry.version == 1:
        LOGGER.error("无法升级 v0.3 之前创建的模型。请删除并重新创建它们。")
        return False
    LOGGER.debug("成功迁移到版本 %s", config_entry.version)
    return True
    ...
    async def async_process(
        ...
        for agent in agent_manager.async_get_agent_preferences():
            ...
            try:
                ...
                LOGGER.exception("使用代理程序进行错误处理 %s", agent.id)
